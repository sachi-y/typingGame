(function (window, document) {
    ...
    let flagX = true; // 経過時間の表示
    let flags = [flagR, flagK, flagG, flagW, flagS, flagX];
    ...
    function start() {
        ...
        flagS = flags[4];
        flagX = flags[5];
        flags = [flagR, flagK, flagG, flagW, flagS, flagX];
    ...
    function gameInit() {
        ...
        //入力時間の表示
        if (flagX) {
            const requiredTime = document.getElementById('required-time');
            requiredTime.style.display = 'block';
            requiredTime.innerHTML = '経過時間:0秒';
            const id = setInterval(() => {
                let time = new Date() - begin;
                // let speed = correct / time * 1 * 1000;
                if (playing) {
                    requiredTime.innerHTML ='経過時間:'+ convTime(time);
                } else {
                    clearInterval(id);
                    requiredTime.innerHTML = '';
                    requiredTime.style.display = 'none';
                }
            }, 100);
        }
